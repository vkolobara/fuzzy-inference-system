...
NotClause::NotClause(shared_ptr<Clause> clause, shared_ptr<BaseOperator::Complement> complement) : clause(
...
OrClause::OrClause(const vector<shared_ptr<Clause>> clauses, shared_ptr<BaseOperator::SNorm> snorm) : clauses(
        clauses), snorm(snorm) {}
...
AndClause::AndClause(vector<shared_ptr<Clause>> clauses, shared_ptr<BaseOperator::TNorm> tnorm) : clauses(
        clauses), tnorm(tnorm) {}